    ...
    with vs.variable_scope(scope or "gru_cell"):
      ...
      with vs.variable_scope("candidate"):
        c = self._activation(_linear([inputs, r * state],
                                     ...
                                     scope=scope))
      new_h = u * state + (1 - u) * c
    return new_h, new_h, u, r, array_ops.zeros_like(r)
...
class BasicLSTMCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or "basic_lstm_cell"):
      ...
      return new_h, new_state, sigmoid(f + self._forget_bias), sigmoid(i), sigmoid(o)
...
class DropoutWrapper(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    output, new_state, g1, g2, g3 = self._cell(inputs, state, scope)
    if (not isinstance(self._output_keep_prob, float) or
        self._output_keep_prob < 1):
      output = nn_ops.dropout(output, self._output_keep_prob, seed=self._seed)
    return output, new_state, g1, g2, g3
