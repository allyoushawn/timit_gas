...
def _rnn_step(
  ...
  def _copy_some_through(flat_new_output, flat_new_gate1, flat_new_gate2, flat_new_gate3, flat_new_state):
    ...
    flat_new_gate1 = [
        _copy_one_through(zero_output, new_gate1)
        for zero_output, new_gate1 in zip(flat_zero_output, flat_new_gate1)]
    flat_new_gate2 = [
        _copy_one_through(zero_output, new_gate2)
        for zero_output, new_gate2 in zip(flat_zero_output, flat_new_gate2)]
    flat_new_gate3 = [
        _copy_one_through(zero_output, new_gate3)
        for zero_output, new_gate3 in zip(flat_zero_output, flat_new_gate3)]
    return flat_new_output + flat_new_gate1 + flat_new_gate2 + flat_new_gate3 + flat_new_state
  ...
  if skip_conditionals:
    ...
    new_output, new_state, new_gate1, new_gate2, new_gate3 = call_cell()
    nest.assert_same_structure(state, new_state)
    new_state = nest.flatten(new_state)
    new_output = nest.flatten(new_output)
    new_gate1 = nest.flatten(new_gate1)
    new_gate2 = nest.flatten(new_gate2)
    new_gate3 = nest.flatten(new_gate3)
    final_output_and_state = _copy_some_through(new_output, new_gate1, new_gate2, new_gate3, new_state)
  else:
  ...
  if len(final_output_and_state) != len(flat_zero_output) * 4  + len(flat_state):
    raise ValueError("Internal error: state and output were not concatenated "
                     "correctly.")
  final_output = final_output_and_state[:len(flat_zero_output)]
  final_gate1 = final_output_and_state[len(flat_zero_output) : len(flat_zero_output) * 2]
  final_gate2 = final_output_and_state[len(flat_zero_output) * 2: len(flat_zero_output) * 3]
  final_gate3 = final_output_and_state[len(flat_zero_output) * 3: len(flat_zero_output) * 4]
  final_state = final_output_and_state[len(flat_zero_output) * 4:]

  for output, flat_output in zip(final_output, flat_zero_output):
    output.set_shape(flat_output.get_shape())
  for substate, flat_substate in zip(final_state, flat_state):
    substate.set_shape(flat_substate.get_shape())
  for gate1, flat_gate1 in zip(final_gate1, flat_zero_output):
    gate1.set_shape(flat_gate1.get_shape())
  for gate2, flat_gate2 in zip(final_gate2, flat_zero_output):
    gate2.set_shape(flat_gate2.get_shape())
  for gate3, flat_gate3 in zip(final_gate3, flat_zero_output):
    gate3.set_shape(flat_gate3.get_shape())

  final_output = nest.pack_sequence_as(
      structure=zero_output, flat_sequence=final_output)
  final_state = nest.pack_sequence_as(
      structure=state, flat_sequence=final_state)
  final_gate1 = nest.pack_sequence_as(
      structure=zero_output, flat_sequence=final_gate1)
  final_gate2 = nest.pack_sequence_as(
      structure=zero_output, flat_sequence=final_gate2)
  final_gate3 = nest.pack_sequence_as(
      structure=zero_output, flat_sequence=final_gate3)

  return final_output, final_state, final_gate1, final_gate2, final_gate3
...
def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
  ...
  with vs.variable_scope(scope or "rnn") as varscope:
    ...
    (outputs, final_state, gate1s, gate2s, gate3s) = _dynamic_rnn_loop(
        cell,
        inputs,
        state,
    ...
    if not time_major:
      ...
      flat_gate1 = nest.flatten(gate1s)
      flat_gate2 = nest.flatten(gate2s)
      flat_gate3 = nest.flatten(gate3s)
      flat_output = [array_ops.transpose(output, [1, 0, 2])
                     for output in flat_output]
      flat_gate1 = [array_ops.transpose(output, [1, 0, 2])
                     for output in flat_gate1]
      flat_gate2 = [array_ops.transpose(output, [1, 0, 2])
                     for output in flat_gate2]
      flat_gate3 = [array_ops.transpose(output, [1, 0, 2])
                     for output in flat_gate3]
      outputs = nest.pack_sequence_as(
          structure=outputs, flat_sequence=flat_output)
      gate1s = nest.pack_sequence_as(
          structure=gate1s, flat_sequence=flat_gate1)
      gate2s = nest.pack_sequence_as(
          structure=gate2s, flat_sequence=flat_gate2)
      gate3s = nest.pack_sequence_as(
          structure=gate3s, flat_sequence=flat_gate3)
    return (outputs, final_state, gate1s, gate2s, gate3s)
...
def _dynamic_rnn_loop(cell,
  ...
  gate1_ta = tuple(_create_ta("gate1_%d" % i,
                               _infer_state_dtype(dtype, state))
                    for i in range(len(flat_output_size)))
  gate2_ta = tuple(_create_ta("gate2_%d" % i,
                               _infer_state_dtype(dtype, state))
                    for i in range(len(flat_output_size)))
  gate3_ta = tuple(_create_ta("gate3_%d" % i,
                               _infer_state_dtype(dtype, state))
                    for i in range(len(flat_output_size)))

  def _time_step(time, output_ta_t, state, gate1_ta_t, gate2_ta_t, gate3_ta_t):
    ...
    if sequence_length is not None:
      (output, new_state, gate1, gate2, gate3) = _rnn_step(
          time=time,
          sequence_length=sequence_length,
    ...
    gate1 = nest.flatten(gate1)
    gate2 = nest.flatten(gate2)
    gate3 = nest.flatten(gate3)

    output_ta_t = tuple(
        ta.write(time, out) for ta, out in zip(output_ta_t, output))
    gate1_ta_t = tuple(
        ta.write(time, out) for ta, out in zip(gate1_ta_t, gate1))
    gate2_ta_t = tuple(
        ta.write(time, out) for ta, out in zip(gate2_ta_t, gate2))
    gate3_ta_t = tuple(
        ta.write(time, out) for ta, out in zip(gate3_ta_t, gate3))

    return (time + 1, output_ta_t, new_state, gate1_ta_t, gate2_ta_t, gate3_ta_t)

  _, output_final_ta, final_state, gate1_final_ta, gate2_final_ta, gate3_final_ta = control_flow_ops.while_loop(
      cond=lambda time, *_: time < time_steps,
      body=_time_step,
      loop_vars=(time, output_ta, state, gate1_ta, gate2_ta, gate3_ta),
      parallel_iterations=parallel_iterations,
      swap_memory=swap_memory)

  # Unpack final output if not using output tuples.
  final_outputs = tuple(ta.stack() for ta in output_final_ta)
  final_gate1s = tuple(ta.stack() for ta in gate1_final_ta)
  final_gate2s = tuple(ta.stack() for ta in gate2_final_ta)
  final_gate3s = tuple(ta.stack() for ta in gate3_final_ta)
  ...
  for output, output_size in zip(final_gate1s, flat_output_size):
    shape = _state_size_with_prefix(
        output_size, prefix=[const_time_steps, const_batch_size])
    output.set_shape(shape)
  for output, output_size in zip(final_gate2s, flat_output_size):
    shape = _state_size_with_prefix(
        output_size, prefix=[const_time_steps, const_batch_size])
    output.set_shape(shape)
  for output, output_size in zip(final_gate3s, flat_output_size):
    shape = _state_size_with_prefix(
        output_size, prefix=[const_time_steps, const_batch_size])
    output.set_shape(shape)
  ...
  final_gate1s = nest.pack_sequence_as(
      structure=cell.output_size, flat_sequence=final_gate1s)
  final_gate2s = nest.pack_sequence_as(
      structure=cell.output_size, flat_sequence=final_gate2s)
  final_gate3s = nest.pack_sequence_as(
      structure=cell.output_size, flat_sequence=final_gate3s)

  return (final_outputs, final_state, final_gate1s, final_gate2s, final_gate3s)
